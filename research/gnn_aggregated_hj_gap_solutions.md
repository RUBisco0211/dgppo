# 保留 GNN 邻域聚合时修复 Multi-Agent Deep-QP HJ Gap 的方案

更新时间：2026-08-30

## 1. 结论

可以继续使用当前的 GNN，对每个智能体的整个局部邻域进行信息聚合，并输出一个 node-wise 的 HJ Safety Value。问题不在于“用了 GNN”，而在于：当前 Safety Value 依赖邻居状态，但方向导数的参数化和训练输入只显式包含本智能体动作。于是，相同的局部图和 ego 动作在不同邻居动作下会产生不同的下一状态与监督目标，当前网络只能把这些差异吸收到一个有混叠的标量项中。

建议的主修复方案是：

1. 保留整邻域 GNN Safety Value，不退回 pair-wise HJ Value。
2. 将方向导数头改成 local-joint-action affine head：同时输出 ego 动作系数和所有可控邻居的动作系数。
3. 单独、off-policy 地训练 HJ Safety Critic；训练样本使用完整的局部联合动作。
4. 在分布式执行时，对未知邻居动作做最坏情况解析消元，最终仍然为每个智能体保留一个只优化 ego 动作的 box + half-space QP。
5. 使用平滑感知边界、短历史或 recurrent GNN，分别处理邻居进出图造成的不连续和局部观测非 Markov 问题。
...
当前 `DeepQPSafetyNet` 先用 `GraphTransformerGNN` 聚合邻居信息，然后由同一个 ego embedding 输出两个 Safety Value、一个 ego 动作系数和一个标量项。可以把当前的结构概括为：
...
在代码中，`coefficient` 的形状只覆盖每个智能体自己的动作维度；loss 中也只计算同索引的 `coefficient * batch.actions`。然而，GNN 输出的 `V_i` 和 `a_ii` 已经依赖邻居状态。实际局部图转移应当写成：
...
所以现有的 `project_box_halfspace` 可以继续使用。与当前实现不同的是，`d_i` 和 `a_ii` 是在包含邻居动作系数的 robust HJ 参数化下学到的，不再被迫把邻居动作影响当成不可辨识噪声。
...
### 5.3 避免 policy distribution leakage
...
|---|---:|---|---|
| 只含 ego action 的当前实现 | 是 | 对固定邻居策略的隐式平均 | derivative target 有混叠，换策略会漂移 |
...
当前工程优先推荐 robust neighbor elimination，因为它同时满足：不改 reward、不依赖解析动力学、保留 GNN 高阶邻域建模、RL 训练继续 CTDE、执行时每个智能体只解自己的局部 QP。
...
### 8.1 `dgppo/algo/module/deep_qp_safety.py`

- 保留 `GraphTransformerGNN` 和 twin Value heads。
- 把单个 `CoefficientHead` 拆成 ego coefficient head 和 agent-agent edge coefficient head。
- edge coefficient 必须由 edge 两端 embedding、edge feature 和整图上下文共同生成，并使用 padding/节点类型 mask。
...
### 8.2 `dgppo/algo/informarl_deep_qp.py`
...
实现 context-conditioned edge coefficient head、joint-action HJ loss 和相应 mask；先用 cooperative target 检查可辨识性和拟合误差。
