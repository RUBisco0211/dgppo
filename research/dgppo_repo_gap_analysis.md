...
> 审阅日期：2026-08-25。本文把本地实现、ICLR 2025 原论文和截至该日期的直接后续工作放在一起判断。文献细节与 primary sources 见 [dgppo_literature_gaps.md](./dgppo_literature_gaps.md)。本次只新增研究笔记，没有修改训练代码；未跟踪的 `rl_on_manifold/` 保持原状。
...
### 3.1 最推荐：Approximation-aware / uncertainty-calibrated DGPPO
...
### 3.4 Density-shift、heterogeneous 与 coupled dynamics
...
## 4. 当前 manifold 分支：方向是对的，但现状不能宣称 hard safety
...
### 4.5 train/eval/test 的 filter state 不一致
...
### 4.6 shield 与长时域策略的关系还没成为 DGPPO 贡献
...
### 5.2 当前 checkpoint 与 `test.py` 默认查找不一致
...
### 5.3 `test.py` 没有完整恢复环境/算法配置

- 创建环境时没有从 config 恢复 `n_rays`，`full_observation` 也由测试 CLI 默认 `False` 覆盖训练配置：[test.py](../test.py#L39)。
- stochastic test wrapper 的函数签名与 `test_rollout` 的调用约定不一致：[test.py](../test.py#L80)、[trainer/utils.py](../dgppo/trainer/utils.py#L70)。
- manifold 算法没有走 stateful `eval_rollout_with_filter`。

### 5.4 末端 next-state safety 没有纳入指标
...
仓库没有 unit/regression test 或 CI。至少应补：
...
当前源码能通过 `python3 -m compileall`，但语法检查无法发现上述协议与数学语义问题。
